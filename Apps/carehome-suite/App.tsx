...
const CopyScheduleModal: FC<{isOpen: boolean, onClose: () => void, shifts: Shift[], onShiftsAdd: (newShifts: Omit<Shift, 'id'>[]) => void, currentDate: Date }> = 
    ...
    const exampleTargetWeek = new Date(currentDate);
    exampleTargetWeek.setDate(currentDate.getDate() - currentDate.getDay());
    
    const exampleSourceWeek = new Date(currentDate);
    exampleSourceWeek.setMonth(currentDate.getMonth() - 1);
    exampleSourceWeek.setDate(currentDate.getDate() - currentDate.getDay());
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Copy Schedule">
            <div className="space-y-4">
                ...
                <p className="text-sm text-gray-400">For example, it will replace the schedule for the week of <span className="font-semibold text-indigo-400">{formatDate(exampleTargetWeek)}</span> with the schedule from the week of <span className="font-semibold text-indigo-400">{formatDate(exampleSourceWeek)}</span>.</p>

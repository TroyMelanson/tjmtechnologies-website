...
const CopyScheduleModal: FC<{isOpen: boolean, onClose: () => void, shifts: Shift[], onShiftsAdd: (newShifts: Omit<Shift, 'id'>[]) => void, currentDate: Date }> = 
    ...
    // Compute example dates using copies to avoid mutating currentDate
    const exampleTargetWeekStart = new Date(currentDate);
    exampleTargetWeekStart.setDate(exampleTargetWeekStart.getDate() - exampleTargetWeekStart.getDay());
    
    const exampleSourceWeekStart = new Date(currentDate);
    exampleSourceWeekStart.setMonth(exampleSourceWeekStart.getMonth() - 1);
    exampleSourceWeekStart.setDate(exampleSourceWeekStart.getDate() - exampleSourceWeekStart.getDay());
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Copy Schedule">
            <div className="space-y-4">
                ...
                <p className="text-sm text-gray-400">For example, it will replace the schedule for the week of <span className="font-semibold text-indigo-400">{formatDate(exampleTargetWeekStart)}</span> with the schedule from the week of <span className="font-semibold text-indigo-400">{formatDate(exampleSourceWeekStart)}</span>.</p>

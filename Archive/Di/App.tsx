...
import { User, AppData } from './types.ts';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // connected to a database (e.g., on Azure).
    try {
      ...
      if (savedData) {
        ...
        // Migration for adding users to appData if they don't exist
        if (!parsedData.users) {
          parsedData.users = INITIAL_DATA.users;
        }
